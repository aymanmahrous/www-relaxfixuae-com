import { createFileRoute } from “@tanstack/react-router”; import { useRef, useState } from “react”; import { SiteHeader } from “@/components/site-header”; import { useI18n } from “@/lib/i18n”; import { Upload, Play, Scissors, Download, Sparkles, Film, Zap, MessageCircle } from “lucide-react”; import { useSettings, waUrl } from “@/lib/settings”; import { videoRequestMessage } from “@/lib/orderMessage”;

export const Route = createFileRoute(”/video”)({ head: () => ({ meta: [ { title: “Video Studio — Relax Fix UAE” }, { name: “description”, content: “Upload, trim, and request professional video edits, reels, and montages. AI-assisted video production for UAE brands in Arabic and English.” }, { property: “og:title”, content: “Video Studio — Relax Fix UAE” }, { property: “og:description”, content: “Trim your clip and request a pro reel, montage, or social cut from our team.” }, { property: “og:url”, content: “https://www.relaxfixuae.com/video” }, ], links: [ { rel: “canonical”, href: “https://www.relaxfixuae.com/video” }, ], }), component: VideoPage, });

function VideoPage() { const { t, lang } = useI18n(); const { settings } = useSettings(); const [url, setUrl] = useState<string | null>(null); const [name, setName] = useState(””); const [duration, setDuration] = useState(0); const [start, setStart] = useState(0); const [end, setEnd] = useState(0); const ref = useRef(null);

function onFile(f: File) { const u = URL.createObjectURL(f); setUrl(u); setName(f.name); }

function onLoaded() { const d = ref.current?.duration ?? 0; setDuration(d); setStart(0); setEnd(d); }

function preview() { if (!ref.current) return; ref.current.currentTime = start; ref.current.play(); const tick = () => { if (!ref.current) return; if (ref.current.currentTime >= end) ref.current.pause(); else requestAnimationFrame(tick); }; requestAnimationFrame(tick); }

function downloadOriginal() { if (!url) return; const a = document.createElement(“a”); a.href = url; a.download = name || “video.mp4”; a.click(); }

return (     {t(“video_title”)}  {t(“video_sub”)}
